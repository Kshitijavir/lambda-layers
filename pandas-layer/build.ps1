# Build script for pandas Lambda Layer
# Run this in PowerShell

$ProjectRoot = Split-Path -Parent $MyInvocation.MyCommand.Path
$LayerZip = Join-Path $ProjectRoot "pandas-layer.zip"

Write-Host "=== Building pandas Lambda Layer ===" -ForegroundColor Cyan

# Step 1: Build the Docker image
Write-Host "[1/3] Building Docker image..." -ForegroundColor Yellow
docker build -t pandas-layer-builder -f "$ProjectRoot\Dockerfile" "$ProjectRoot"
...
docker run --rm --entrypoint sh -v "${ProjectRoot}:/host" pandas-layer-builder -c "cp /layer.zip /host/pandas-layer.zip"
...
if (Test-Path $LayerZip) {
    ...
    & docker run --rm --entrypoint sh -v "${ProjectRoot}:/host" pandas-layer-builder -c "unzip -l /host/pandas-layer.zip | head -20"
} else {
...
Write-Host "You can now upload 'pandas-layer.zip' to AWS Lambda as a layer." -ForegroundColor Green
